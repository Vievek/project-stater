...
describe('UserRepository (Unit)', () => {
  ...
  beforeEach(() => {
    ...
    repository = new UserRepository(mockDbClient, mockCacheService);
  });

  describe('findById', () => {
    it('should fetch from db and cache with custom 30s TTL', async () => {
      const mockUser = buildUser({ id: '1' });
      (mockDbClient.findUnique as jest.Mock).mockResolvedValue(mockUser);
  ...
  describe('findAllLatest', () => {
    test.each([
      ...
      const mockUsers = buildUserList(3);
      (mockDbClient.findMany as jest.Mock).mockResolvedValue(mockUsers);

      await repository.findAllLatest(page ? { pagination: { page, pageSize } } : undefined);
      
      const expectedCacheKey = page
        ? `user:repo:findAllLatest:${JSON.stringify({ pagination: { page, pageSize } })}`
        : 'user:repo:findAllLatest';
      expect(mockCacheService.getOrSet).toHaveBeenCalledWith(
        expectedCacheKey,
        expect.any(Function),
        60,
