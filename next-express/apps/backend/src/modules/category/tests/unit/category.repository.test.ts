...
describe('CategoryRepository (Unit)', () => {
  ...
  beforeEach(() => {
    ...
    repository = new CategoryRepository(mockDbClient, mockCacheService);
  });

  describe('findById', () => {
    it('should fetch from db and cache with custom 30s TTL', async () => {
      const mockCategory = buildCategory({ id: '1' });
      (mockDbClient.findUnique as jest.Mock).mockResolvedValue(mockCategory);
  ...
  describe('findAllLatest', () => {
    test.each([
      ...
      const mockCategorys = buildCategoryList(3);
      (mockDbClient.findMany as jest.Mock).mockResolvedValue(mockCategorys);

      await repository.findAllLatest(page ? { pagination: { page, pageSize } } : undefined);
      
      const expectedCacheKey = page
        ? `category:repo:findAllLatest:${JSON.stringify({ pagination: { page, pageSize } })}`
        : 'category:repo:findAllLatest';
      expect(mockCacheService.getOrSet).toHaveBeenCalledWith(
        expectedCacheKey,
        expect.any(Function),
        60,
